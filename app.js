...
axios.get('https://hook.us1.make.com/rx33x2dsf5v8fkq0aj1r3c4onapvj6ka').then(async res => {
  ...
  for (const Order of reportData) {
    let orderData = Order.Order[0];
    await base('Report Data').create({
      'date/time': orderData.LastUpdatedDate[0],
      'type': 'order',
      'order id': orderData.AmazonOrderID[0],
      'merchant order id': orderData.MerchantOrderID[0],
      'order status': orderData.OrderStatus[0],
      ...
      'asin': orderData.OrderItem[0].ASIN[0],
      'sku': orderData.OrderItem[0].SKU[0],
      'item status': orderData.OrderItem[0].ItemStatus[0],
